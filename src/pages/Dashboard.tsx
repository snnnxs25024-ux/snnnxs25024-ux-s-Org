...
const Dashboard: React.FC<DashboardProps> = ({ workers, attendanceHistory, refreshData, setAttendanceHistory, autoOpenSessionId, clearAutoOpenSessionId }) => {
    ...
    // Derived values for backward compatibility and internal logic
    const viewingDate = useMemo(() => {
        if (filterType === 'month') {
            ...
            return new Date(dateRange.start + 'T00:00:00');
        }
        return new Date();
    }, [filterType, selectedDate, selectedMonth, selectedYear, dateRange]);

    const viewingMonth = useMemo(() => {
        const now = new Date();
        const currentMonthStr = `${now.getFullYear()}-${(now.getMonth() + 1).toString().padStart(2, '0')}`;
        
        if (filterType === 'month' && selectedMonth === currentMonthStr) {
            return null;
        }
        return { month: viewingDate.getMonth(), year: viewingDate.getFullYear() };
    }, [filterType, selectedMonth, viewingDate]);
    ...
    // Filtered attendance history based on selected filter
    const displayedHistory = useMemo(() => {
        return attendanceHistory
            .filter(session => {
                ...
                const sessionMonthStr = `${sessionYear}-${sessionMonth}`;

                if (filterType === 'day') {
                    return sessionDateStr === selectedDate;
                } else if (filterType === 'month') {
                    return sessionMonthStr === selectedMonth;
                } else if (filterType === 'year') {
                    return sessionYear === selectedYear;
                } else if (filterType === 'range') {
                    return sessionDateStr >= dateRange.start && sessionDateStr <= dateRange.end;
                }
                return true;
            })
            .sort((a, b) => {
    ...
    }, [attendanceHistory, filterType, selectedDate, selectedMonth, selectedYear, dateRange]);

    const historyTitle = useMemo(() => {
        if (filterType === 'day') {
            return `Data Kehadiran: ${new Date(selectedDate + 'T00:00:00').toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}`;
        } else if (filterType === 'month') {
            const [y, m] = selectedMonth.split('-');
            const monthName = new Date(parseInt(y), parseInt(m) - 1).toLocaleString('id-ID', { month: 'long' });
            return `Arsip Kehadiran: ${monthName} ${y}`;
        } else if (filterType === 'year') {
            return `Arsip Kehadiran Tahun: ${selectedYear}`;
        } else if (filterType === 'range') {
            const start = new Date(dateRange.start + 'T00:00:00').toLocaleDateString('id-ID', { day: 'numeric', month: 'short' });
            const end = new Date(dateRange.end + 'T00:00:00').toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' });
            return `Data Kehadiran: ${start} - ${end}`;
        }
    ...
    const fulfillmentStats = useMemo(() => {
        const calculate = (startDay: number, endDay: number) => {
            const relevantSessions = displayedHistory.filter(session => {
                const sessionDate = new Date(session.date + 'T00:00:00');
                return sessionDate.getDate() >= startDay && sessionDate.getDate() <= endDay;
            });
        ...
        if (filterType === 'month') {
            const [y, m] = selectedMonth.split('-');
            const monthName = new Date(parseInt(y), parseInt(m) - 1).toLocaleString('id-ID', { month: 'long', year: 'numeric' });
    ...
    }, [displayedHistory, filterType, selectedMonth]);
    ...
    }, [attendanceHistory, workers, viewingDate, reportSearchQuery]);
            ...
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                    <button 
                        ...
                        className="flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-700 font-bold py-2 px-4 rounded-lg border border-gray-200 transition-all duration-200 shadow-sm hover:shadow-md"
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200 border-t-4 border-purple-500 transition-shadow duration-300 hover:shadow-xl">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
                        ...
                        <div className="relative w-full sm:w-64">
                            ...
                            <input
                                type="text"
                                placeholder="Cari Nama / Ops ID..."
                                value={reportSearchQuery}
                                onChange={(e) => setReportSearchQuery(e.target.value)}
                                className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-purple-500 focus:border-purple-500 sm:text-sm transition-all"
                            />
                        </div>
                    </div>
            ...
            >
                <div className="space-y-6">
                    <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            {(['day', 'month', 'year', 'range'] as const).map((type) => (
                                ...
                                >
                                    {type === 'day' ? 'Harian' : type === 'month' ? 'Bulanan' : type === 'year' ? 'Tahunan' : 'Rentang'}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        ...
                        {filterType === 'month' && (
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Pilih Bulan</label>
                                <input 
                                    type="month" 
                                    value={selectedMonth}
                        ...
                        {filterType === 'year' && (
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Pilih Tahun</label>
                                <select 
                                    value={selectedYear}
                        ...
                        {filterType === 'range' && (
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Sampai</label>
                                    <input 
                                        type="date" 
                                        value={dateRange.end}
                    ...
                    <div className="flex justify-end pt-2">
                        <button
                            ...
                            className="bg-blue-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-blue-700 shadow-lg transition-all active:scale-95"
                        >
                            Terapkan Filter
                        </button>
                    </div>
